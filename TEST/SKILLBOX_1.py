...
class Man:
    ...
    def shopping(self):
        if self.house.money >= 50:
            cprint('{} сходил за едой'.format(self.name), color='magenta')
            self.house.money -= 50
            self.house.food += 50
        else:
            cprint('у {} деньги кончились!'.format(self.name), color='red')

    def go_in_to_house(self, house):
        self.house = house
        self.fullness -= 10
        cprint('{} вехал в дом'.format(self.name), color='cyan')
